...
export default function RecommendationPanel({ recommendation, emergencyType, onSendAlert, alertStatus, onAmbulanceLocation }) {
  ...
  const {
    hospitalName, address, score, distanceKm, etaMinutes,
    availableBeds, availableSpecialists, contact, scoreBreakdown, hospitalId,
  } = recommendation;
  ...
  return (
    <div className="recommendation-panel">
      ...
      <div className="rec-contact">
        <span>📞 {contact?.emergencyLine || contact?.phone || 'N/A'}</span>
      </div>

      {/* Alert Button — pass hospitalId explicitly */}
      <AlertButton
      ...
      {alertStatus?.sent && AmbulanceQR && (
        <Suspense fallback={
          <div style={{ color: '#7a8bad', fontSize: '12px', padding: '10px' }}>
            Loading tracker...
          </div>
        }>
          <AmbulanceQR
...
function AlertButton({ hospitalId, onSendAlert, alertStatus }) {
  // ── Already sent ────────────────────────────────────────────────────────────
  if (alertStatus?.sent) {
    return (
  ...
  if (alertStatus?.failed) {
    return (
      <div className="alert-failed">
        ...
        {alertStatus.canRetry && (
          <button
            className="retry-btn"
            onClick={() => onSendAlert(hospitalId, true)}
          >
            🔄 Retry Alert
          </button>
        )}
  ...
  return (
    <button
      ...
      onClick={() => {
        if (!hospitalId) {
          console.error('hospitalId is missing from recommendation');
          return;
        }
        onSendAlert(hospitalId);
      }}
      disabled={alertStatus?.loading || !hospitalId}
    >
      {alertStatus?.loading ? (
