...
// ✅ Global blink loop — runs forever, finds elements by class
let globalBlinkInterval = null;
let blinkState = true;

function startGlobalBlink() {
  if (globalBlinkInterval) return; // already running
  globalBlinkInterval = setInterval(() => {
    blinkState = !blinkState;

    // Blink all best-hospital dots
    document.querySelectorAll('.blink-dot').forEach(el => {
      ...
      el.style.boxShadow = blinkState
        ? '0 0 18px #d93025, 0 0 32px rgba(217,48,37,0.6)'
        : 'none';
    });
...
export default function HospitalMap({ hospitals, recommendation, userLocation, showRoute }) {
  ...
  // Start global blink loop once on mount
  useEffect(() => {
    startGlobalBlink();
  }, []);
  ...
  const updateMarkers = (L, map) => {
    ...
    // ── Best hospital — uses CSS class for global blink ──────────────────
    if (recHospital) {
        ...
        const icon = L.divIcon({
          html: `
            <div style="position:relative;width:48px;height:48px">
              ...
              <div style="position:absolute;top:-26px;left:50%;transform:translateX(-50%);background:#d93025;color:white;font-size:9px;font-weight:800;padding:2px 9px;border-radius:10px;white-space:nowrap;font-family:sans-serif;box-shadow:0 2px 6px rgba(0,0,0,0.25);">⭐ BEST HOSPITAL</div>
            </div>`,
          className: '',
          iconSize: [48, 48],
          iconAnchor: [24, 24],
        ...
        const m = L.marker([lat, lng], { icon }).addTo(map).bindPopup(`
          <div style="font-family:sans-serif;min-width:210px;line-height:1.8;font-size:13px">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:12px">
              <span>🛏 ICU Beds</span>
              <b style="color:#0f9d58">${icuBeds} available</b>
              <span>👨‍⚕️ On Duty</span>
              <span style="font-size:10px">${(recHospital.availableSpecialists||[]).join(', ')||'General'}</span>
              <span>📍 Distance</span>
              <b>${recHospital.distanceKm || recommendation?.distanceKm || '?'} km</b>
              ${recommendation?.etaMinutes
                ? `<span>⏱ ETA</span><b style="color:#f29900">${recommendation.etaMinutes} mins</b>`
                : ''}
            </div>
          </div>`);
    ...
    // ── Patient location — uses CSS class for global blink ───────────────
    if (userLocation) {
  ...
  const clusterHospitals = (list, radiusKm) => {
    ...
    list.forEach((h, i) => {
      ...
      const lat1 = h.location?.coordinates?.[1];
      const lng1 = h.location?.coordinates?.[0];
      if (!lat1 || !lng1) return;
      const group = [h]; visited.add(i);
      list.forEach((h2, j) => {
        if (visited.has(j)) return;
        const lat2 = h2.location?.coordinates?.[1];
        const lng2 = h2.location?.coordinates?.[0];
        if (!lat2 || !lng2) return;
        const dist = Math.sqrt(
          Math.pow((lat1-lat2)*111, 2) +
          Math.pow((lng1-lng2)*111*Math.cos(lat1*Math.PI/180), 2)
        );
  ...
  const makeMarker = (L, h) => {
    ...
    const size = 24;
    const icon = L.divIcon({
      html: `<div style="width:${size}px;height:${size*1.3}px">
        <svg width="${size}" height="${size*1.3}" viewBox="0 0 40 52" xmlns="http://www.w3.org/2000/svg">
          ...
          <text x="20" y="25" text-anchor="middle" font-size="13" font-weight="bold" fill="${color}">${isFull?'✕':'+'}</text>
        </svg>
      </div>`,
  ...
  const drawRoute = async (L, map) => {
    ...
    const iv = setInterval(() => {
      if (step >= coords.length-1) { clearInterval(iv); return; }
